...
// Jeff's Atlas user ID (single-owner system)
const JEFF_ATLAS_USER_ID = '116262192843412290714';
...
async function getPersonContext(slackUserId, slackEmail) {
  try {
    ...
    const { data: bySlack } = await supabase
      ...
      .eq('atlas_user_id', JEFF_ATLAS_USER_ID)
...
function categorizeActivity(person) {
  ...
  if (total > 1000) return 'very active — works closely with Jeff';
  if (total > 200) return 'regular contact with Jeff';
